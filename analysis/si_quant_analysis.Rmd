...
# si.unequal <- full_join(mss.acc, mss, by ="test_age")
# with(si.unequal, t.test(Implicature,some,equal.var=TRUE))
# 
# #then resample our data to get the sample sizes equal - ideally we should repeat this a bunch
...
# 
# #what about by age?
# d.younger <- d.combined %>%
#   filter(age < 4 | test_age < 4)
# 
# 
# d.older <- d.combined %>%
#   filter(age > 4 | test_age > 4)
# 
# with(d.younger, t.test(Implicature,some,equal.var=TRUE))
# with(d.older, t.test(Implicature,some,equal.var=TRUE))
# 
# #performance on none trials, same
# with(d.combined, t.test(None,none,equal.var=TRUE))
# with(d.younger, t.test(Implicature,some,equal.var=TRUE))
# with(d.older, t.test(Implicature,some,equal.var=TRUE))
```
...
```{r}
#add agesplit to large dataframe
d_all %<>%
  mutate(Agesplit = cut(test_age, breaks=c(3, 3.5, 4, 4.5, 5)), 
         Agesplit = factor(Agesplit,labels=c("3-3.5 years", "3.5-4 years", "4-4.5 years", "4.5-5 years")))
...
quartz()
q <- qplot(SI_none, SI_implicature, col=Agesplit, 
	ylab="Proportion of SI 'implicature' trials correct",
	xlab="Proportion of SI 'none' trials correct",
		position=position_jitter(.02), data=d_all) + 
	geom_smooth(method="lm", col="black", lty=1)  +
	geom_smooth(aes(col=Agesplit, group=Agesplit), 
				se=FALSE, method="lm",lty=3) + coord_cartesian(ylim = c(0, 1)) 
...
#GQ none and SI none
quartz()
qplot(GQ_none_correct, SI_none, col=Agesplit, 
	ylab="Proportion of SI 'none' trials correct",
	xlab="Proportion of GQ 'none' trials correct",
		position=position_jitter(.009), data=d_all) + 
	geom_smooth(method="lm", col="black", lty=1) + 
			theme_bw()+
	geom_smooth(aes(col=Agesplit, group=Agesplit), 
				se=FALSE, method="lm",lty=3)
...
#GQ some and SI none
quartz()
qplot(GQ_some_correct, SI_none, col=Agesplit, 
	ylab="Proportion of SI 'none' trials correct",
	xlab="Proportion of GQ 'some' trials correct",
		position=position_jitter(.001), data=d_all) +
	geom_smooth(method="lm", col="black", lty=1) + 
			theme_bw()+
	geom_smooth(aes(col=Agesplit, group=Agesplit), 
				se=FALSE, method="lm",lty=3) + coord_cartesian(ylim = c(0, 1)) 
...
#GQnone and SI some
quartz()
qplot(GQ_none_correct, SI_implicature, col=Agesplit, 
	ylab="Proportion of SI 'implicature' trials correct",
	xlab="Proportion of GQ 'none' trials correct",
		position=position_jitter(.001), data=d_all) +
	geom_smooth(method="lm", col="black", lty=1) + 
